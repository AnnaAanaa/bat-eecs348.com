...
double evaluateExpression(const string& expression, unordered_map<string, double>& variables) {
    ...
    stack<char> unaryOperators;  // Stack to track unary operators
    ...
    for (size_t i = 0; i < expression.size(); ++i) {
        if (isspace(expression[i])) {
            ...
            // Check for unary operators following the closing parenthesis
            while (!unaryOperators.empty()) {
                char unaryOp = unaryOperators.top();
                unaryOperators.pop();
                double operand = operands.top();
                operands.pop();
                bool error;
                operands.push(applyOperator(unaryOp, 0, operand, error));
                if (error) {
                    return 0;
                }
            }
            ...
            if ((expression[i] == '-' || expression[i] == '+') &&
                ...
                unaryOperators.push(expression[i]);
            } else {
